...
impl DomainRepository {
    ...
    fn claim_clock(
        ...
        // ADR-182 decision 3, inside the profiled (`?2 IS NOT NULL`) block: a
        // dispatch whose session's engagement has an open fence is not a
...
/// `cleared_by` is the caller's own route word, recorded on any agent fence
/// this resolution clears (ADR-182 decision 3).
#[allow(clippy::too_many_arguments)]
