...
## 2026-09-13 — The native agent roster is a bounded projection (ADR-126)

- `GET /console/api/agents` behind the existing `authenticate` hoop, no
  scope: scope facts are a payload on reads, never a gate. One writer job
  (`DomainRepository::agent_roster`) — engagements joined to resources for
  the framework, newest `runner_attempts.created_at` per engagement for
  `last_activity_ms` ("last dispatch activity", not last seen); a dispatch
  with no attempt row reports `null`, never zero.
- Exactly seven scalar keys per item and no nested object, so no credential
  home, workdir, state dir, workspace path, tmux target, pane buffer or
  token can travel inside one; the client's exact-key validator
  (`validateAgents`) fails the whole read on an eighth key.
- Server-owned `unavailable` names the eight columns native has no source
  for; the page renders whatever the server names, so a future source turns
  a column on by removing its name server-side.
- The page `/console/agents/` stages through `build-native-console.mjs`
  with the `assets.rs` mime arm, key map and `/console/agents` alias; the
  required key `/console/usage/` is unchanged and the five-document
  exception stays five — the roster serves as a non-document taking no
  query string. The rail's workforce row links here; the retained arm of
  `app/agents/page.jsx` redirects to `/workforce`, the retained roster's
  own home.
- Tests: `native_console_agent_roster_observation` (exact seven-key set,
  null-not-zero pinned on AlertWorker/PageWorker, the unavailable list,
  `assert_private`), `native_console_agent_roster_refuses_foreign_origin`
  (`console_origin_required` before any store read, no agents key in any
  refusal body, non-GET refused by the router), and
  `native_console_agent_roster_browser` (default-off feature, listed under
  `--all-features`; the harness owns the access ticket and the driver mints
  nothing).
- Gates in this sandbox: `cargo fmt`, clippy `-p hagency --all-targets
  --features native-console-browser -- -D warnings`, `node --check` on both
  touched scripts, and the rust spec-binding inventory pass.
  `cargo test -p hagency --locked --test console` cannot run here: the
  fixture's `Console::load` fails with `Err(Assets)` for every selector —
  17/17 including 14 untouched by this diff, and the same failure
