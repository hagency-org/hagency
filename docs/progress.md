## 2026-09-13 — Admitted-corpus retention bound (brief 1, ADR-125)

- The admitted-message corpus gains its bound: `admitted_messages` and its
  per-session children are pruned by a pinned sweep (`sweep_admitted_corpus`,
  one `Immediate` transaction per tick, batch 512) instead of only ever
  growing to the 100 000 refusal. Pruned rows — and their
  `matrix_ingress_events` provenance — move into a bounded
  `retained_message_archive` (keyed `UNIQUE(engagement_id, source_key)`,
  ceiling-bounded, pruned oldest-first in the same tick), so an exact
  redelivery of a pruned admission still dedupes and a divergent one is still
  refused (the four ingress reads answer live first, archive on a live miss;
  both callers reconstruct `wake`/`config` from the archive row).
- The pin rule is the ADR's ten clauses: P1 recency, P2 unprocessed input,
  P3' claimed-but-unprocessed (bounded by `processed_at` — `dispatch_id` is
  never cleared), P4/P5 live and unknown-fate dispatch custody read from
  `runner_dispatches.state` directly (never the `unresolved_dispatches`
  view), P6'/P7' open canonical task (status `done` releases —
  `task_intents.state='closed'` has no production writer), P9/P10 attachment
  custody. Migration 026 (all `CREATE ... IF NOT EXISTS`, replay-safe over the
  rewound fixture) adds the archive, the seven pin-probe indexes and the one
  shared receipt table `retention_prune_receipts`; the fourteen schema-head
  assertions moved to 26 in the same commit.
- The sweep runs as the `messages` phase of ONE retention tick
  (`start_retention_sweep`, `RETENTION_SWEEP_PERIOD` 60 s, per-phase budget
  600 ms, `elapsed_ms` logged every tick, batch-halving reduction rule, one
  receipt row per phase per tick trimmed to 100); the ceiling-alert sweep
  stays its own task. `retention_status()` is the one operator read;
  `MESSAGE_RETENTION_CEILING` 5000 / floor 100 with the `Bootstrap` override.
- Oracle: `native/scripts/corpus-retention-vectors.mjs` executes the REAL
  retained `planMessagePrune` (imported with a one-shot express stub — no
  node_modules in this sandbox) and writes
  `tests/fixtures/corpus-retention-vectors.json`; the vector covers the
  shared recency/inbox/membership subset only, and says so. Store tests:
  `native/hagency-store/tests/retention.rs` (ten scenarios, spec-bound);
  this sandbox's store suite dies at `Repository::open` (EPERM), so the
  orchestrator runs it at integration.
