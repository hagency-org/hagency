...
**Placement and scope.** The strip is `mockup/components/NativeStatusStrip.jsx`, rendered as a `PageHead` child (`PageHead.jsx:19-29` already takes children) on every native page — readiness is a fleet fact, and the cutover operator may be on any page when a 503 flips. No scope, no route, no session: the same class as the rail. **Restart control stays explicitly out of scope until the service wrapper exists** — the retained proxy's own rule ("nothing here can … start a process") is inherited verbatim.
